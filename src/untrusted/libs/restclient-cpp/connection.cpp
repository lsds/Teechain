...
RestClient::Connection::performCurlRequest(const std::string& uri) {
  ...
  // force TLS v1.2
  curl_easy_setopt(this->curlHandle, CURLOPT_USE_SSL, CURLUSESSL_ALL); 
  ...
  curl_easy_setopt(this->curlHandle, CURLOPT_ERRORBUFFER, errbuf);
  ...
  if (res != CURLE_OK) {
    switch (res) {
      ...
      default:
        ...
        std::cout << "Error buffer: " << std::string(errbuf, CURL_ERROR_SIZE) << std::endl;
