...
struct BlockHasher
{
    ...
    size_t operator()(const uint256& hash1, const uint256& hash2) const {
        return hash1.GetCheapHash() < hash2.GetCheapHash();
    }
};
...
typedef std::map<uint256, CBlockIndex*, BlockHasher> BlockMap;
...
/* CONSTANTS IMPORTED FROM MAIN.CPP */
...
/* END CONSTANTS IMPORTED FROM MAIN.CPP */
